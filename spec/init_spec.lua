...
describe('preview', function()
  ...
  describe('result', function()
    it('returns nil when no compile result exists', function()
      local bufnr = helpers.create_buffer({})
      assert.is_nil(preview.result(bufnr))
      helpers.delete_buffer(bufnr)
    end)
  end)
  ...
  describe('unnamed buffer guard', function()
    ...
    it('output warns no compiler output on unnamed buffer', function()
      local bufnr = helpers.create_buffer({}, 'lua')
      local msg = capture_notify(function()
        preview.output(bufnr)
      end)
      assert.is_not_nil(msg)
      assert.is_truthy(msg:find('no compiler output'))
      helpers.delete_buffer(bufnr)
    end)
  end)
end)
