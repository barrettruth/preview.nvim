...
function M.compile(bufnr)
  ...
  if not name then
    vim.notify('[preview]: no provider configured for this filetype', vim.log.levels.WARN)
    return
  end
...
function M.open(bufnr)
  ...
  if not compiler.open(bufnr, open_config) then
    vim.notify('[preview]: no output file available for this buffer', vim.log.levels.WARN)
  end
end
...
function M.output(bufnr)
  ...
  if not compiler.output(bufnr) then
    vim.notify('[preview]: no compiler output available for this buffer', vim.log.levels.WARN)
  end
end
